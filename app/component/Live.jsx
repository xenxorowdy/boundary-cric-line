  ...
  function parseSessionData(sessionString) {
    try {
      const sessionPattern =
        /(\d+) Over \((\d+-\d+)\) (\d+) Runs\/(\d+)wk \((\d+-\d+) ([A-Z]+)\)/g;
      const sessions = [];
      let match;
    ...
    <View style={styles.container}>
        ...
        >
          <View style={styles.tvStyle}>
            ...
            <ShowAnimation
              ...
              value={matchDetail?.first_circle ?? ""}
            />
            <Text
              style={{
                position: "absolute",
          ...
          <View style={[styles.boxtv]}>
            ...
            >
              ...
              <CusText style={{ fontWeight: "600" }}>
                {matchDetail?.battingScore}
                {/* {matchDetail?.team_a} */}
                {/* {team_a_score} */}
              </CusText>
            ...
            >
              ...
              <CusText style={{ fontWeight: "600", flexWrap: "wrap" }}>
                {matchDetail?.secbattingScore}
                {/* {matchDetail?.team_a} */}
                {/* {team_a_score} */}
              </CusText>
            </View>
      ...
      >
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text style={styles.TextColor}>
            Run Rate: {`${matchDetail?.curr_rate?.toString() ?? "-"}`}
          </Text>
          <Text style={styles.TextColor}>
            Target: {matchDetail?.target?.toString() || "-"}
          </Text>

          {matchDetail?.rr_rate?.toString() && (
            <Text style={styles.TextColor}>
              RRR: {matchDetail?.rr_rate?.toString()}
            </Text>
          )}
        </View>
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text style={styles.TextColor}>
            Run Needed: {matchDetail?.run_need?.toString() || "-"}
          </Text>

          <Text style={styles.TextColor}>
            Ball Remaning: {matchDetail?.ball_rem?.toString()}
          </Text>
        </View>
      </View>
      <View
      ...
      >
        <View style={[styles.box, { borderRadius: 10 }]}>
          <Text style={[styles.TextColor, { fontWeight: "600", fontSize: 16 }]}>
            Winning Chances:
          </Text>
          <View style={{ flexDirection: "row", gap: 10, alignItems: "center" }}>
            <Text style={styles.TextColor}> {matchDetail?.fav_team}</Text>
            <Text style={[styles.TextColor, styles.minStyle]}>
              {ratefetch(matchDetail?.min_rate)}{" "}
            </Text>
            <Text style={[styles.TextColor, styles.maxStyle]}>
              {ratefetch(matchDetail?.max_rate)}
            </Text>
          </View>
        </View>
        ...
        {matchDetail?.lambi_min && (
          ...
          >
            ...
            >
              <Text style={styles.TextColor}>
                {matchDetail?.current_inning}
                {inningsuffix(matchDetail?.current_inning)} Inning's Total Runs:
              </Text>
            </View>
            <View
              style={{
      ...
      >
          ...
          renderItem={({ item }) => (
            <View style={[styles.rowBoxback]}>
              ...
              >
                ...
                <Text style={[[styles.TextColor, { width: 35 }]]}>
        ...
        <View style={styles.rowBoxback}>
          <CusText>
            Partnership: {matchDetail?.partnership?.run?.toString() ?? "-"} (
            {matchDetail?.partnership?.ball?.toString() ?? "-"})
          </CusText>

          {matchDetail?.lastwicket?.player && (
            <CusText>
              Last Wkt:{" "}
              {matchDetail?.lastwicket?.player?.toString() +
                " " +
                matchDetail?.lastwicket?.run?.toString() +
                "(" +
                matchDetail?.lastwicket?.ball?.toString() +
                ")"}{" "}
            </CusText>
          )}
        </View>
      ...
      <View style={{ flexDirection: "column", padding: 0, flex: 1 }}></View>
      <View
        style={{
          flexDirection: "column",
          padding: 10,
